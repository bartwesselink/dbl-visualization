...
Draws a circle with the given center point and radius. The `mode` indicates the shader to use to draw the circle. The `mainColor` and `extraColor` are passed to the shader for use. If the shader mode is set to the filled circle variant the `extraColor` will be ignored and the `mainColor` used to draw the circle. If the shader mode is set to the drawn circle variant the `extraColor` will be ignored and the `mainColor` used to draw the circle. If the shader mode is set to the liend circle variant the `mainColor` will be used to fill the circle and the `extraColor` will be used to draw the outline.
