...
export class SimpleTreeMap implements Visualizer {
    public draw(input: VisualizerInput): Draw[] {
        ...
        const drawTree = (tree: Node, bounds: Bounds, internalNode: boolean, color: number[]): void => {
            ...
            if (drawOutlines) {
                draws.push({ type: 6 /** FillLinedAAQuad **/, identifier: tree.identifier, options: { x: bounds.left, y: bounds.bottom, width: width, height: height, fillColor: color, lineColor: [0, 0, 0, 1] } });
            } else {
